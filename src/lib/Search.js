...
const searchObject = (object, string, properties) => { // {author:{fullName: true}, book: {name}}
    let keys = properties ? Object.keys(properties) : Object.keys(object)
    for (const key of keys) {
        ...
        if (typeof object[key] === "object" && searchObject(object[key], string, properties && properties[key])) return true
        if (Array.isArray(object[key]) && searchArray(object[key], string, properties && properties[key]).length > 0) return true
    }
    return false
}